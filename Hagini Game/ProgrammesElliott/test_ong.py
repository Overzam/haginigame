...
track = pyg.transform.scale(track, (2560 * width / 1536, 4040 * height / 864)).convert()
track_border = pyg.image.load('img/track_border.png')
track_border = pyg.transform.scale(track_border, (2560 * width / 1536, 4040 * height / 864)).convert_alpha()
track_border_mask = pyg.mask.from_surface(track_border)

checkpoint = pyg.image.load('img/finish.png')
checkpoint = pyg.transform.scale(checkpoint, (int(50), int(250))).convert_alpha()
checkpoint_horizontal = pyg.transform.rotate(checkpoint, 90)
checkpoint_horizontal_1 = pyg.transform.scale(checkpoint_horizontal, (int(240), int(50))).convert_alpha()
checkpoint_horizontal_2 = pyg.transform.scale(checkpoint_horizontal, (int(210), int(50))).convert_alpha()
checkpoint_horizontal_3 = pyg.transform.scale(checkpoint_horizontal, (int(600), int(50))).convert_alpha()
...
player = Tank((width/2, height/2), 6, 2, image, 50)

# Création de l'instance fond d'écran/circuit
x_offset, y_offset = -1240,-3320
circuit = Background((x_offset,y_offset), 6, 2, track, track_border_mask)
...
objets = [[[[0,0],track_border,(0,1,0,0,None)],
          ...
          [[[1905,3060],crosshair,(0,0,0,0,1)],[[1960,2890],crosshair,(0,0,0,0,1)],[[1870,2780],crosshair,(0,0,0,0,1)],[[1990,2705],crosshair,(0,0,0,0,1)],[[1830,2605],crosshair,(0,0,0,0,1)],[[1930,2540],crosshair,(0,0,0,0,1)],[[1790,2440],crosshair,(0,0,0,0,1)],
          [[2005,1560],crosshair,(0,0,0,0,2)],[[2195,1435],crosshair,(0,0,0,0,2)],[[2070,1360],crosshair,(0,0,0,0,2)],[[2180,1265],crosshair,(0,0,0,0,2)],[[2320,1210],crosshair,(0,0,0,0,2)],[[2245,1060],crosshair,(0,0,0,0,2)],[[2360,955],crosshair,(0,0,0,0,2)]],
...
#objets= [[[width*i[0][0]/1100, height*i[0][1]/600], i[1], i[2]] for i in objets]
...
while run:
    ...
    countdown = round(30 - ((pyg.time.get_ticks() - start_ticks) / 1000))
    ...
    screen.blit(print_pv, (width/2 + 100,0))
